(function() {
	...
	function addEventListeners() {
		...
		document.getElementById('save-filter').addEventListener( 'click', filterKeyOn );
		document.getElementById('save-contact').addEventListener( 'click', newContactHandler );		
	}

	function filterKeyOn()
	{
		alert('Debug Message!');
	}
	...
	{
		// create data as a json string
		var fstName = document.getElementById('new-firstname').value;
		var lstName = document.getElementById('new-lastname').value;
		var obj = '{'
       +'"firstname" : "'+fstName+'",'
       +'"lastname"  : "'+lstName+'",'
       +'"address" : false'
	...
	function redrawTodosUI() {
		...
		var todo, checkbox, label, deleteLink, divDisplay, inputEditTodo, li, i, l, contactDetail,
		...
		for ( i = 0, l = todos.length; i < l; i++ ) {
			todo = todos[ i ];
			contactDetail =  todos[ i ].data;
			console.log(contactDetail);
			
			// convert contactDetail(string) to json object 
			var json = JSON.stringify(eval("(" + contactDetail + ")"));
			...
			label.appendChild( document.createTextNode( todo.data ) );
